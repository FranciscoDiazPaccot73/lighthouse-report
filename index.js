...
const argv = { url: 'https://www.franciscodiazpaccot.dev' }
...
if (argv.from && argv.to) {
  ...
  launchChromeAndRunLighthouse(argv.url).then(async results => {
    const prevReports = await glob(`${dirName}/*.json`, {
      sync: true
    });
    ...
    if (prevReports.length) {
      ...
      prevReports.forEach(report => {
        dates.push(
          new Date(path.parse(report).name.replace(/_/g, ":"))
        );
      })
